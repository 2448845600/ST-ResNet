...
from datasets.preprocessing.MaxMinNormalization import MinMaxNormalization
from datasets.preprocessing.STMatrix import STMatrix
from datasets.preprocessing.timestamp import timestamp2vec


def load_holiday(timeslots, data_root):
    """
    载入假期数据
    :param timeslots:
    :param fname:
    ...
    """
    fname = os.path.join(data_root, 'BJ_Holiday.txt')
    f = open(fname, 'r')
...
def load_meteorol(timeslots, data_root):
    '''
    ...
    气象数据
    '''
    fname = os.path.join(data_root, 'BJ_Meteorology.h5')
    f = h5py.File(fname, 'r')
    Timeslot = f['date'][:]
...
def load_dataset(
    ...
    if holiday_data:
        # load holiday
        holiday_feature = load_holiday(timestamps_Y, data_root)
        meta_feature.append(holiday_feature)
    if meteorol_data:
        # load meteorol data
        meteorol_feature = load_meteorol(timestamps_Y, data_root)
...
def load_data(data_root, cache_path, len_closeness, len_period, len_trend, len_test):
    ...
    if os.path.exists(fname):
        ...
        X_train, Y_train, X_test, Y_test, mmn, external_dim, timestamp_train, timestamp_test = \
            load_dataset(data_root, cache_path, len_closeness=len_closeness, len_period=len_period, len_trend=len_trend,
